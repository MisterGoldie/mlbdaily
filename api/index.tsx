...
app.frame('/games/:index', async (c) => {
  ...
  try {
    ...
    if (!games || games.length === 0) {
      return c.res({
        image: 'https://placehold.co/1200x630/png?text=No+Data+Available&font-size=24',
        intents: [
    ...
    if (!game) {
      return c.res({
        image: 'https://placehold.co/1200x630/png?text=Game+Not+Found&font-size=24',
        intents: [
          <Button action="/">Back to Start</Button>
        ]
      })
    ...
    // Format the text with explicit line breaks and significantly decreased font size
    const imageText = `${awayTeam}%0Avs%0A${homeTeam}%0A${statusInfo}%0A${gameNumberInfo}`

    console.log('Image text:', imageText)

    return c.res({
      image: `https://placehold.co/1200x630/png?text=${imageText}&font-size=24`,
      intents: [
    ...
    return c.res({
      image: 'https://placehold.co/1200x630/png?text=Error+Occurred&font-size=24',
      intents: [
        <Button action="/">Back to Start</Button>
      ]
    })
